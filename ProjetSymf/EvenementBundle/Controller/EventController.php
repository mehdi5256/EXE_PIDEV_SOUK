...
class EventController extends Controller
{
    ...
    public function CommentAction(Request $request){
         ...
         $comment = new EvenementComment();

         $form_comment = $this->createForm('EvenementBundle\Form\CommentType', $comment);

         $form_comment->handleRequest($request);

         $user_=$em->getRepository(\EvenementBundle\Entity\UserE::class)->find($idu);
         ...
         if ($form_comment->isSubmitted() && $form_comment->isValid()) {
             ...
             $comment->SetEvent($detail);

             $em->persist($comment);

             $em->flush();

             return $this->redirectToRoute('detail', array('id' => $detail->getId()));
}
